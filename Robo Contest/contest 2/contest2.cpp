...
int main(int argc, char** argv) {
	...
	std::cout<< "initposex: "<< initposex<< " || " << "initposey: "<< initposey<< " || " << "initposephi: "<< initposephi<< "\n"<<std::endl;
        ...
        boxx = boxes.coords[i][0]+0.7*cos(boxes.coords[i][2]);
        boxy = boxes.coords[i][1]+0.7*sin(boxes.coords[i][2]);
    ...
    for (int i = 0; i < results.size(); i++){
        f << "Tag: " << results[i] << "    Box Coordinates: " << boxes.coords[i][0] << ", " << boxes.coords[i][1] << std::endl;
    }
    f.close();
    return 0;
}
